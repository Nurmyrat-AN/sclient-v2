...
const authMiddleware = async (req, res, next) => {
    try {
        ...
        if (req._parsedUrl.pathname.indexOf('/api/devices/') !== 0 && req._parsedUrl.pathname.indexOf('/api') === 0 && !req.hasAccessToApp) throw CustomError.badRequest({ status: 403, message: 'Access denied!' })
